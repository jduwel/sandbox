    ...
    <xsl:template match="/">
        ...
        <nts:contentAsserts>
            <xsl:variable name="idExpressionParts">
                <nts:idExpressions>
                    <xsl:for-each select="f:Bundle/f:entry/f:resource/f:*[local-name()=$scenarioResources]">
            ...
            <xsl:choose>
                <xsl:when test="count($combinedIdExpressionParts/nts:idExpression) = count(distinct-values($combinedIdExpressionParts/nts:idExpression))">
                    <xsl:for-each select="$combinedIdExpressionParts/nts:idExpression">
                        ...
                        <f:variable>
                            <f:name value="{@name}"/>
                            <f:expression value="Bundle.entry.select(resource as {@resource}).where({./text()}).id"/>
            ...
            <!-- Generate 1 assert per variable to check if variable isn't empty or contains more then 1 resource -->
